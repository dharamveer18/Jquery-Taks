$(document).ready(function () {
  ...
  if (data.length > 0) {
    data.forEach(heading => {
      ...
      $(".main-content").append(`<div class="heading-container"><h2>${heading.heading}</h2>
        <button type="button" class="close" style="border:2px solid #ccc; border-radius:50%; margin:10px">X</button>
        </div>`);
$('.close').click(function(){
  ...
  location.reload(true);
      ...
      heading.subheadings.forEach(subheading => {
        $(`h2:contains('${heading.heading}')`).after(`<div class="subheading-container"><h3 style="margin-left:30px;">${subheading.subheading}</h3></div>`);
        subheading.inputs.forEach(input => {
          const inputHTML = getInputHTML(input.type, input.attributes);
          $(`h2:contains('${heading.heading}')`).nextAll(".subheading-container").find(`h3:contains('${subheading.subheading}')`).after(
  ...
  $(".heading-form").on("submit", function (e) {
    ...
    if (newHeading) {
      ...
      $(".main-content").append(`<div class="heading-container"><h2>${newHeading}</h2></div>`);
      $(".heading-input").val("");
  ...
  $(".subheading-form").on("submit", function (e) {
    ...
    if (selectedHeading && newSubheading) {
      ...
      $(`h2:contains('${selectedHeading}')`).after(`<div style="margin-left:30px;" class="subheading-container"><h3>${newSubheading}</h3></div>`);
  ...
  $(".heading-form-select").on("change", function () {
    ...
    const subheadings = $(`h2:contains('${selectedHeading}')`).nextAll(".subheading-container").map(function () {
  ...
  $(".form-inputs").on("submit", function (e) {
    ...
    if (selectedHeading && selectedSubheading && inputType) {
      ...
      $(`h2:contains('${selectedHeading}')`).nextAll(".subheading-container").find(`h3:contains('${selectedSubheading}')`).after(
